...
import { Users, Lock, Shield, UserPlus, Activity } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] animate-fade-in">
      <div className="glass-card p-12 text-center max-w-lg w-full" style={{ border: '1px solid rgba(6, 182, 212, 0.2)' }}>
        ...
        <p className="text-sm mb-8" style={{ color: '#71717a' }}>Admin-only user and role management panel</p>
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            { icon: <UserPlus className="w-4 h-4" />, label: 'Create Users', desc: 'Add new team members' },
            { icon: <Shield className="w-4 h-4" />, label: 'RBAC', desc: 'Admin, Analyst, Viewer' },
            { icon: <Activity className="w-4 h-4" />, label: 'Activity Log', desc: 'Per-user audit trail' },
            { icon: <Lock className="w-4 h-4" />, label: 'Access Control', desc: 'Enable/disable accounts' },
          ].map((item, i) => (
